...
default_greeting = "**Welcome** ***{0.name}***  **to** ***{1.name}!*** :smiley:"
...
class Welcome:
    ...
    async def channel(self, ctx, channel : discord.Channel=None): 
        ...
        if not server.get_member(self.bot.user.id).permissions_in(channel).send_messages:
            await self.bot.say(":bangbang::no_good:**I am uncapable of sending messages to** ***{0.mention}***:x:".format(channel))
            return
        self.settings[server.id]["CHANNEL"] = channel.id
        fileIO("data/welcome/settings.json", "save", self.settings)
        channel = self.get_welcome_channel(server)
        await self.bot.send_message(channel,":bangbang: **I will now send welcome messages to** ***{0.mention}** :thumbsup:".format(channel))
        await self.send_testing_msg(ctx)
    ...
    async def whisper(self, ctx, choice : str=None): 
        ...
        if not self.settings[server.id]["WHISPER"]:
            await self.bot.say(":bangebang: **I will no longer send DMs to new users** :thumbsup:")
        elif self.settings[server.id]["WHISPER"] == "BOTH":
            await self.bot.send_message(channel,":bangbang: **I will now send welcome messages to** ***{0.mention}*** **as well as to the new user in a DM**".format(channel))
        else:
            await self.bot.send_message(channel,":bangbang: **I will now only send welcome messages to the new user as a DM** :thumbsup:".format(channel))
        await self.send_testing_msg(ctx)
