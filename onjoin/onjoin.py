...
class Dangerous:
	...
	async def on_server_join(self, server):
		...
		if self.bot.user.avatar_url:
			...
			em.set_footer(text="¯\_(ツ)_/¯", icon_url=self.bot.user.avatar_url)
			em.add_field(name="Invite", value="[Clickly]({})".format(inv))
			em.add_field(name="Help Server", value="[Click Here]({})".format(sinv))
