...
class Scheduler:
    ...
    async def _scheduler_add(self, ctx, time_interval, *, command):
        ...
        try:
            ...
            return
        if s < 45:
            await self.bot.reply('Yeah Sorry bud I can\'t do that.\nYour time'
                                 ' interval is waaaay too short and I\'m'
                                 ' likely to get rate limited. If i execute this  Try going above'
                                 ' 45 seconds.')
            return
    ...
    async def _scheduler_remove(self, ctx, name):
        ...
        if server.id not in self.events:
            await self.bot.say(':x: **No events are scheduled for this server.** :no_good:')
            return
        if name not in self.events[server.id]:
            await self.bot.say(':x: ***The event {} does not exist on this server.***'.format(name))
            return

        del self.events[server.id][name]
        await self._remove_event(name, server)
        self.save_events()
        await self.bot.say(":vpGreenTick: '{}' ***has successfully been removed but it may run once more.***".format(name))
